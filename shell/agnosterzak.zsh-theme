...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    prompt_segment blue white "%{$fg_bold[white]%(!.%{%F{white}%}.)%}$USER@%m%{$fg_no_bold[white]%}"
  else
    prompt_segment yellow blue "%{$fg_bold[blue]%(!.%{%F{blue}%}.)%}@$USER%{$fg_no_bold[blue]%}"
  fi
}
...
prompt_dir() {
  prompt_segment cyan black "%{$fg_bold[black]%}%~%{$fg_no_bold[black]%}"
}
